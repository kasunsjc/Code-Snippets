...
Options:
  --demo <none|eventhub|storage-queue|all>  Optional demo workload deployment.
...
if [[ "$DEMO" != "none" && "$DEMO" != "eventhub" && "$DEMO" != "storage-queue" && "$DEMO" != "all" ]]; then
  echo "ERROR: Invalid --demo value '$DEMO'. Expected one of: none, eventhub, storage-queue, all."
  exit 1
fi
...
kubectl get secret azure-storage-secret -n "$K8S_NAMESPACE" >/dev/null && echo "  - azure-storage-secret"
kubectl get secret azure-eventhub-secret -n "$K8S_NAMESPACE" >/dev/null && echo "  - azure-eventhub-secret"
