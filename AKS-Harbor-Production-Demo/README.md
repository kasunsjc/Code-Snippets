...
You need an SSH keypair for the jumpbox before the first deploy:

```bash
ssh-keygen -t ed25519 -f ./harbor-jumpbox-key -N ""
# put the .pub contents in terraform.tfvars as jumpbox_ssh_public_key
export JUMPBOX_SSH_KEY="$(pwd)/harbor-jumpbox-key"   # deploy.sh reads this env var
```

To poke around interactively instead: `az network bastion ssh --name <bastion_name> --resource-group <rg> --target-resource-id <jumpbox_vm_id> --auth-type ssh-key --username azureuser --ssh-key ./harbor-jumpbox-key` (all four values are Terraform outputs). The jumpbox's cloud-init already installs `az`, `kubectl`, and `helm`.
...
- An SSH keypair for the jumpbox (`ssh-keygen -t ed25519 -f ./harbor-jumpbox-key -N ""`) — the public key goes in `terraform.tfvars` as `jumpbox_ssh_public_key`
- Terraform >= 1.6.0
...
# edit terraform.tfvars: dns_zone_name, dns_zone_resource_group, acme_email, jumpbox_ssh_public_key
cd ..
export JUMPBOX_SSH_KEY=./harbor-jumpbox-key   # private key matching jumpbox_ssh_public_key
./deploy.sh
```
...
| `ssh`/`scp` to the jumpbox fails with `Permission denied (publickey)` | `JUMPBOX_SSH_KEY` doesn't match the public key in `terraform.tfvars` | Regenerate both together (`ssh-keygen ...`), re-apply Terraform, and re-export `JUMPBOX_SSH_KEY` before re-running `deploy.sh` |
